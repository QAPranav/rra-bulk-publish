@redirects @rra-bulk-publish
Feature: Redirect Validation - RRA Bulk Publish Pages
  As a Senior QA Engineer
  I want to validate all old RRA pages redirect correctly to new pages
  So that users and search engines reach the right content after the 1st May 2026 go-live

  Background:
    Given the base URL is configured

  @redirect-301
  Scenario Outline: Old path "<oldPath>" returns 301 and redirects to a valid page
    When I validate the redirect for "<oldPath>"
    Then the redirect should return 301 and resolve correctly
