@seo @rra-bulk-publish
Feature: SEO & Metadata Validation - RRA Bulk Publish Pages
  As a Senior QA Engineer
  I want to validate SEO metadata on all new RRA pages
  So that search engines correctly index the new content after the 1st May 2026 go-live

  Background:
    Given the base URL is configured

  @seo-existence
  Scenario Outline: Page "<URL>" has required SEO meta tags
    Given I navigate to "<URL>"
    When I check SEO meta tags on the page
    Then the page should have required SEO meta tags
  ...
  @seo-title
  Scenario Outline: Page "<page>" meta title contains "<expectedTitle>"
    Given I navigate to "<page>"
    Then the meta title should contain "<expectedTitle>"
  ...
  @seo-description
  Scenario Outline: Page "<page>" meta description contains "<expectedPhrase>"
    Given I navigate to "<page>"
    Then the meta description should contain "<expectedPhrase>"
  ...
  @seo-og
  Scenario Outline: Page "<page>" has correct Open Graph values
    Given I navigate to "<page>"
    Then the og:title should contain "<expectedOgTitle>"
    And the og:url should contain "<expectedOgUrl>"
    And the og:description should not be empty

    Examples:
      | page | expectedOgTitle | expectedOgUrl |
      # ── Add rows here once content team confirms expected values ──
