@vrt @rra-bulk-publish
Feature: Visual Regression Testing - RRA Bulk Publish Pages
  As a Senior QA Engineer
  I want to visually compare RRA pages against approved baselines
  So that I can catch unintended visual changes after the 1st May 2026 content migration

  @vrt-desktop
  Scenario Outline: Page "<URL>" matches its visual baseline at desktop viewport
    Given I am using the "desktop" viewport
    And I navigate to "<URL>"
    When I capture and compare a screenshot
    Then the page should match its visual baseline
  ...
  @vrt-mobile
  Scenario Outline: Page "<URL>" matches its visual baseline at mobile viewport
