@links @rra-bulk-publish
Feature: Link Validation - RRA Bulk Publish Pages
  As a Senior QA Engineer
  I want to validate all internal, external and anchor links on updated RRA pages
  So that users can navigate without encountering broken or dead links after the 1st May 2026 go-live

  Background:
    Given the base URL is configured

  @link-check
  Scenario Outline: All links on "<URL>" are valid
    Given I navigate to "<URL>"
    When I check all links on the page
    Then all internal links should return HTTP 200
    And all external links should be reachable
    And all anchor links should resolve to existing elements
