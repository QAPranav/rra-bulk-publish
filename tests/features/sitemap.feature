@sitemap @rra-bulk-publish
Feature: Sitemap Validation - RRA Bulk Publish Pages
  As a Senior QA Engineer
  I want to ensure that old RRA pages are not present in the sitemap
  So that search engines do not index outdated URLs after the 1st May 2026 go-live

  Background:
    Given the base URL is configured

  @sitemap-old-pages
  Scenario Outline: Old path "<oldPath>" is absent from the sitemap
    When I check that "<oldPath>" is absent from the sitemap
    Then "<oldPath>" should not appear in the sitemap
